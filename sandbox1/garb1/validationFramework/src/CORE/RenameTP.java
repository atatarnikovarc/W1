...
public class RenameTP {
	private static final String pathToJmxs = "c:\\worked\\testplans";
	private static final String pathToUpdatedJmxs = "c:\\worked\\updated";


	public static void main(String[] args) throws Exception {
		System.out.println("----next iteration");
		File workingDir = new File(RenameTP.pathToJmxs);
				...
				for (int j = 0, max1 = currentList.getLength(); j < max1; j++) { // cycle
					...
					for (int k = 0, max2 = currMap.getLength(); k < max2; k++) {
						...
						if (currMap.item(k).getNodeName().equals("class")
							...
							System.out.println("test plan elm finded!");
							isTestPlan = true;
							break;
						}
					}
					if (isTestPlan) {
						...
						Node searchedNode = getNodeByAttr(testElmChilds,
								"property", "name", "TestElement.name");
						searchedNode.setTextContent(remExt(jmxFilesList[i].getName()));
						++collectorIndex;
	...
	private static void saveXml(Document doc, String fileName) {
		...
		File xml = new File(RenameTP.pathToUpdatedJmxs + "\\" + fileName);
