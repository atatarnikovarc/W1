...
public class MakePixel extends TestCase {
    ...
    MakePixel ( Selenium browserFromMain,
    		...
    		String scripFolder,
    		String htmlFolder,
    		String srcPrefixHttp) {
    	HTDOCS_SCRIPT_PATH  = scripFolder;
    	HTDOCS_HTML_PATH = htmlFolder;
    	SRC_PREFIX_HTTP = srcPrefixHttp;
	...
	private void fillFormDataPixel (
    	...
    	try {
			File dir = new File(HTDOCS_SCRIPT_PATH);
			String[] list = dir.list();
		    if (list.length == 0) {
		    	System.out.println("Error. Have no .js Scripts at " + HTDOCS_SCRIPT_PATH);
		    	return;
		    }
		    File file;
		    String filename;
		    for (int i = 0; i < list.length; i++) {
		    	file = new File(HTDOCS_SCRIPT_PATH + list[i]);
		    	filename = file.getName();
				int end = filename.lastIndexOf(".");
				//end--;
				char scriptname[] = new char[end]; 
				filename.getChars(0, end, scriptname, 0);
				makeDataPixelHTMLTemplate(pixelNumber, dataPixelFormat, String.valueOf(scriptname));
		    }
    	} catch (Exception e) {
    		System.out.println(e.getMessage() + "\r\n" +
    				"possible reason: have no .js scripts as " + HTDOCS_SCRIPT_PATH);
            e.printStackTrace();
    	}
	...
	private void makeDataPixelHTMLTemplate (
			String pixelNumber, String dataPixelFormat, String scriptName
			) throws Exception {
    	try {
//    		File path = new File(HTDOCS_HTML_PATH);
//    		path.mkdirs();
//    		FileOutputStream fos = new FileOutputStream(HTDOCS_HTML_PATH +
//    				pixelNumber + "_" + dataPixelFormat + "_" + scriptName + ".html");
//	    	PrintWriter DataPixelTemplateFile = new PrintWriter(fos);

	    	String icScript = "";
	    	if ( scriptName.equals("parts_async_ic") ) {
	    		icScript = "ic";
	    	}
	    	
	    	String scriptType = "";
	    	if ( dataPixelFormat.contentEquals("S") ) {
	    		scriptType = "script";
	    	} else if ( dataPixelFormat.contentEquals("I") ) {
	    		scriptType = "image";
	    	} else if ( dataPixelFormat.contentEquals("F") ) {
	    		scriptType = "frame";
	    	} else {
	    		System.out.println("ERROR: Wrong param: dataPixelFormat= " + dataPixelFormat);
	    		System.exit(1);
	    	}

	    	String scriptBody = "\r\n\t<script type=\"text/javascript\">\r\n" +
		    			"\t\t_" + icScript + "prot=\"http\";\r\n" +
		    			"\t\t_" + icScript + "pixel=" + pixelNumber + ";\r\n" +
		    			"\t\t_ch=\"\";\r\n" +
		    			"\t\t_type=\"" + scriptType + "\";\r\n" +
		    			"\t</script>\r\n\r\n" +
		    			"\t<script type=\"text/javascript\" " +
		    			"src=\"" + SRC_PREFIX_HTTP + scriptName + ".js\">\r\n" +
		    			"\t</script>\r\n\r\n";
	    	
    		File path = new File(HTDOCS_HTML_PATH);
    		path.mkdirs();
	    	if ( scriptName.equals("parts") ) {
	    		FileOutputStream fos = new FileOutputStream(HTDOCS_HTML_PATH +
	    				pixelNumber + "_" + dataPixelFormat + "_" + scriptName + "_HEAD.html");
		    	PrintWriter DataPixelTemplateFile = new PrintWriter(fos);
		    	DataPixelTemplateFile.println(
		    			"<HTML>\r\n" +
		    			"\t<HEAD>\r\n" +
		    			"\t\t<TITLE>" + scriptType + " Pixel " + scriptName + "</TITLE>\r\n" +
		    			scriptBody +
		    			"\t</HEAD>\r\n" +
		    			"\t<BODY>\r\n" +
		    			"\t\t<h1>" + scriptType + " Pixel " + scriptName + " id=" + pixelNumber + "</h1>\r\n" +
		    			"\t</BODY>\r\n" +
		    			"</HTML>\r\n"
		    	);
		    	DataPixelTemplateFile.close();
	    		fos = new FileOutputStream(HTDOCS_HTML_PATH +
	    				pixelNumber + "_" + dataPixelFormat + "_" + scriptName + "_BODY.html");
		    	DataPixelTemplateFile = new PrintWriter(fos);
		    	DataPixelTemplateFile.println(
		    			"<HTML>\r\n" +
		    			"\t<HEAD>\r\n" +
		    			"\t\t<TITLE>" + scriptType + " Pixel " + scriptName + "</TITLE>\r\n" +
