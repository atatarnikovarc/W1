...
class LogSummarizer(object):
  ...
  AD_ERROR_MESSAGES = {
      'no-types': (
          'none of { video_url, html_snippet, creative_id, '
          ' snippet_template } are set, exactly one must be set.'),
      'mulitple-types': (
          'more than one of { video_url, html_snippet, creative_id, '
          ' snippet_template } are set, exactly one must be set.'),
  ...
  def ValidateAd(self, ad, ad_index, record):
    ...
    """
    ad_type_fields = [
        'html_snippet', 'video_url', 'creative_id', 'snippet_template']
    found_types = []
