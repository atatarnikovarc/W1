...
def GetGoogleUserIdsFromFile(google_user_ids_filename):
  """Reads Google user IDs from the given file and returns them in a list.

  Args:
    google_user_ids_filename: A filename, the file should contain one Google
    user ID per line.

  Returns:
    A list of Google user IDs read from the file or None if there were problems
    reading the file.
  """
  if not os.path.exists(google_user_ids_filename):
    print 'Invalid file: %s' % google_user_ids_filename
    return None

  try:
    google_user_ids_file = open(google_user_ids_filename, 'r')
    google_user_ids = google_user_ids_file.readlines()
    google_user_ids_file.close()
  except IOError:
    print 'Could not open file for reading: %s' % google_user_ids_filename
    return None
  google_user_ids = map(lambda x: x.rstrip('\r\n'), google_user_ids)
...
def main():
  ...
  if opts.google_user_ids_file:
    google_user_ids = GetGoogleUserIdsFromFile(opts.google_user_ids_file)
